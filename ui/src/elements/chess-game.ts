 ...
 */
 export class ChessGame extends ScopedRegistryHost(MobxLitElement) {
  ...
  async getGameInfo(retriesLeft = 4): Promise<GameEntry> {
    try {
      const gameInfo = await this._store.chess.getGame(this.gameHash);
      return gameInfo;
    } catch (e) {
  ...
  async firstUpdated() {
    ...
    this._moves = await this._store.chess.getGameMoves(this.gameHash);

    const opponent = gameInfo.players.find(
      player => player !== this.myAddress
    ) as string;

    await this._store.profiles.fetchAgentProfile(opponent);
  ...
  get myAddress() {
    return serializeHash(this._store.chess.cellId[1]);
  }
  ...
  getOpponentNickname(): string {
    return this._store.profiles.profileOf(this.getOpponent()).nickname;
  }
  ...
  async makeMove(move: ChessMove) {
    ...
    const hash = await this._store.chess.makeMove(
      this.gameHash,
      previousMoveHash,
      move
    );
    ...
    if (this.isGameOver()) {
      ...
      await this._store.chess.publishResult(result);
    }
